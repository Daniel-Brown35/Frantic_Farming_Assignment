...
public class PickupControllerTutorial : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.F))
        {
            if (heldAnimal == null)
            {
                ...
                {
                    if (hit.transform.gameObject.tag == "Animal" && hit.transform.gameObject.GetComponent<WalkTutorialFrantic>().canBePickedUp == true)
                    {
    ...
    void DropAnimal()
    {
        ...
        {
            if (heldAnimal.GetComponent<WalkTutorialFrantic>().inPenArea == true)
            {
                heldAnimal.GetComponent<NavMeshAgent>().enabled = true;
                heldAnimalRB.useGravity = true;
                heldAnimalRB.drag = 1;
                heldAnimalRB.constraints = RigidbodyConstraints.None;
                heldAnimal.GetComponent<WalkTutorialFrantic>().Repen();
                heldAnimal.transform.parent = null;
                heldAnimal = null;
            }
        }
